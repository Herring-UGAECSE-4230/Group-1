...
main:
    ...
    vcvt.s32.f32 s2, s0    @ Convert input to integer in s2 for comparison
    cmp s2, #0             @ Check if input is zero
...
factorial:
    ...
    vcvt.s32.f32 s2, s0    @ Convert n to integer in s2 for comparison
    cmp s2, #0             @ Check if n == 0
